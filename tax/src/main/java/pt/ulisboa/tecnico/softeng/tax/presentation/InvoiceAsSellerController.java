...
@Controller
@RequestMapping(value = "/tax/payers/{nif}/invoicesAsSeller")
public class InvoiceAsSellerController {
	private static Logger logger = LoggerFactory.getLogger(InvoiceAsSellerController.class);
	...
	public String invoiceForm(Model model, @PathVariable String nif) {
		...
		model.addAttribute("invoices", TaxInterface.getInvoiceAsSellerDataList(nif));
		return "invoicesAsSellerView";
	}
	...
	public String invoiceSubmit(Model model, @PathVariable String nif, @ModelAttribute InvoiceData invoiceData) {
		...
		return "redirect:/tax/payers/" + nif + "/invoicesAsSeller";
	}
