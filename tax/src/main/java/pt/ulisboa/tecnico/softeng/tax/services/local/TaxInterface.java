...
public class TaxInterface {
	...
	public static List<InvoiceData> getInvoiceDataList(String nif) {
		...
		invoiceList.addAll( taxPayer.getInvoiceOfSellerSet().stream().map(i -> new InvoiceData(i))
					.sorted((i1, i2) -> i1.getSellerNif().compareTo(i2.getSellerNif())).collect(Collectors.toList()) );
		invoiceList.addAll( taxPayer.getInvoiceOfBuyerSet().stream().map(i -> new InvoiceData(i))
